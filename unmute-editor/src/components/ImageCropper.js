...
import { setImageRef, setMinValue, setRatio, updateZoomValue } from '../features/image/imageSlice';
...
const ImageEditor = ({ image, orientation, passepartout, cropData, unmute, activeUnmute, swiperRef }) => {
  const dispatch = useDispatch();
  const params = useParams();
  const cropperRef = useRef(null);
  const [isInitialized, setIsInitialized] = useState(false);
  const [crop, setCrop] = useState(unmute.properties._crop_data || { x: 0, y: 0 });
